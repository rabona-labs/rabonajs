    ...
    switch (layer.type) {
      ...
      case "passLayer":
        layer.data.forEach((pass) => {
          const radius = layer.options.radius || 15;
          newLayer.append("line").attr("id", "line").style("stroke", layer.options.color || "magenta").style("stroke-width", layer.options.width || 1.2).attr("x1", pass.startX * this.pitchOptions.scaler + 50).attr("y1", pass.startY * this.pitchOptions.scaler + 50).attr("x2", pass.endX * this.pitchOptions.scaler + 50).attr("y2", pass.endY * this.pitchOptions.scaler + 50).attr("marker-end", "url(#arrow)");
          newLayer.append("circle").attr("cx", pass.startX * this.pitchOptions.scaler + 50).attr("cy", pass.startY * this.pitchOptions.scaler + 50).attr("r", radius).style("fill", layer.options.color);
          newLayer.append("text").text(function(d) {
            return "10";
          }).attr("id", "text").attr("x", pass.startX * this.pitchOptions.scaler + 50).attr("y", pass.startY * this.pitchOptions.scaler + 50).attr("font-family", "sans-serif").attr("font-size", radius + 3).attr("text-anchor", "middle").attr("transform", "translate(0,0)rotate(0)").attr("alignment-baseline", "middle").attr("z-index", 1e3).on("mouseover", (d) => d3.select(d.srcElement.parentNode).raise());
        });
        break;
      case "line":
      default:
