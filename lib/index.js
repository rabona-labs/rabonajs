...
var Pitch = class {
  ...
  addLayer(layer) {
    ...
    layer.options.showArrows ? currentPitch.append("svg:defs").append("svg:marker").attr("id", "arrow").attr("viewBox", "0 -5 10 10").attr("refX", 0).attr("markerWidth", 5).attr("markerHeight", 5).attr("strokeWidth", layer.options.width || 1.2).attr("orient", "auto").append("svg:path").attr("d", "M0,-5L10,0L0,5").style("fill", layer.options.color || "magenta") : null;
    switch (layer.type) {
      case "circle":
      ...
      case "passLayer":
        layer.data.forEach((pass) => {
          ...
          currentPitch.append("circle").attr("cx", pass.startX * this.pitchOptions.scaler + 50).attr("cy", pass.startY * this.pitchOptions.scaler + 50).attr("r", layer.options.circleRadius || 15).style("fill", layer.options.color);
        });
        break;
      case "line":
      default:
