    ...
    switch (layer.type) {
      case "circle":
        layer.data.forEach((record) => {
          ...
          newLayer.append("circle").attr("cx", record.startX * this.pitchOptions.scaler + 50).attr("cy", record.startY * this.pitchOptions.scaler + 50).attr("r", record.radius || 10).style("fill", circleColor);
          options.stroke ? newLayer.style("stroke", options.stroke) : null;
          options.strokeWidth ? newLayer.style("stroke-width", options.strokeWidth) : null;
          if (record.label) {
            newLayer.append("text").text(function() {
              return record.label;
            }).attr("id", "text").attr("x", record.startX * this.pitchOptions.scaler + 50).attr("y", record.startY * this.pitchOptions.scaler + 50).attr("font-family", "sans-serif").attr("font-size", record.radius + 3).attr("fill", "black").attr("text-anchor", "middle").attr("transform", "translate(0,0)rotate(0)").attr("alignment-baseline", "middle").attr("z-index", 1e3).on("mouseover", (d) => d3.select(d.srcElement.parentNode).raise());
          }
        });
        break;
      case "ballMovement":
        layer.data.forEach((record) => {
          ...
          const width = options.getWidth ? (_c = options.getWidth) == null ? void 0 : _c.call(options, record) : layer.options.width;
          layer.options.showArrows ? newLayer.append("svg:defs").append("svg:marker").attr("id", `arrow-${id}`).attr("viewBox", "0 -5 10 10").attr("refX", 0).attr("markerWidth", 5).attr("markerHeight", 5).attr("strokeWidth", width || 1.2).attr("orient", "auto").append("svg:path").attr("d", "M0,-5L10,0L0,5").style("fill", lineColor || "magenta") : null;
          newLayer.append("line").attr("id", "line").style("stroke", lineColor || "magenta").style("stroke-width", width || 1.2).attr("x1", record.startX * this.pitchOptions.scaler + 50).attr("y1", record.startY * this.pitchOptions.scaler + 50).attr("x2", record.endX * this.pitchOptions.scaler + 50).attr("y2", record.endY * this.pitchOptions.scaler + 50).attr("marker-end", `url(#arrow-${id})`);
          ...
          if (record.label) {
            newLayer.append("text").text(function() {
              return record.label;
            }).attr("id", "text").attr("x", record.startX * this.pitchOptions.scaler + 50).attr("y", record.startY * this.pitchOptions.scaler + 50).attr("font-family", "sans-serif").attr("font-size", radius + 3).attr("fill", textColor).attr("text-anchor", "middle").attr("transform", "translate(0,0)rotate(0)").attr("alignment-baseline", "middle").attr("z-index", 1e3).raise();
          }
        });
        break;
      case "line":
