    ...
    switch (layer.type) {
      ...
      case "passLayer":
        layer.data.forEach((pass) => {
          ...
          currentPitch.append("circle").attr("cx", pass.startX * this.pitchOptions.scaler + 50).attr("cy", pass.startY * this.pitchOptions.scaler + 50).attr("r", layer.options.radius || 15).style("fill", layer.options.color);
        });
        break;
      case "line":
      default:
