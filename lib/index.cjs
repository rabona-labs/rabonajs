...
var Pitch = (_class = class {
  ...
  addLayer(layer) {
    ...
    switch (temp.type) {
      case "circle":
        currentPitch.append("circle").attr("cx", temp.data.cx * this.pitchOptions.scaler + 50).attr("cy", temp.data.cy * this.pitchOptions.scaler + 50).attr("r", temp.data.radius || 10).style("fill", temp.options.color);
        break;
      case "passLayer":
        for (const line of temp.data) {
          currentPitch.append("line").style("stroke", temp.options.color || "magenta").style("stroke-width", temp.options.width || 1.2).attr("x1", line.startX * this.pitchOptions.scaler + 50).attr("y1", line.startY * this.pitchOptions.scaler + 50).attr("x2", line.endX * this.pitchOptions.scaler + 50).attr("y2", line.endY * this.pitchOptions.scaler + 50).attr("marker-end", "url(#arrow)");
          currentPitch.append("circle").attr("cx", line.startX * this.pitchOptions.scaler + 50).attr("cy", line.startY * this.pitchOptions.scaler + 50).attr("r", 20).style("fill", temp.options.color);
          currentPitch.append("circle").attr("cx", line.endX * this.pitchOptions.scaler + 50).attr("cy", line.endY * this.pitchOptions.scaler + 50).attr("r", 20).style("fill", temp.options.color);
        }
        break;
      case "point":
      case "line":
